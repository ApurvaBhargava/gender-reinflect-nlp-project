...
from greedy_decoding import Batch_Sampler
import logging
...
class Vectorizer:
    ...
    def vectorize(self, src, trg, src_gender, trg_gender):
        ...
        vectorized_src_char  = self.get_src_indices(src)
        vectorized_trg_x, vectorized_trg_y = self.get_trg_indices(trg)
        vectorized_src_gender = self.src_gender_vocab.lookup_token(src_gender)
    ...
    def from_serializable(cls, contents):
        ...
        src_gender_vocab = SeqVocabulary.from_serializable(contents['src_gender_vocab'])
...
class MT_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        src_gender, trg_gender = example.src_gender, example.trg_gender
        vectorized = self.vectorizer.vectorize(src, trg, src_gender, trg_gender)
        return vectorized
...
class Collator:
    ...
    def __call__(self, batch):
        ...
        src_seqs_genders = [x['src_gender'] for x in sorted_batch]
        ...
        src_seqs_genders = torch.tensor(src_seqs_genders, dtype=torch.long)
