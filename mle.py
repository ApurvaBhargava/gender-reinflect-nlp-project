...
class MLE:
    ...
    def __getitem__(self, sw_tg):
        ...
        if (context, trg_gender) in self.model:
            return dict(self.model[(context, trg_gender)])
...
def inference(model, data_examples, args):
    ...
    for example in data_examples:
        ...
        if inference == trg:
            stats[(src_gender, trg_gender, 'correct')] = 1 + stats.get((src_gender, trg_gender, 'correct'), 0)
        else:
            stats[(src_gender, trg_gender, 'incorrect')] = 1 + stats.get((src_gender, trg_gender, 'incorrect'), 0)
